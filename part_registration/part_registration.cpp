...
{	
	...
	if (scia.hasConverged()) {
		...
		double factor_a = 2; //Õâ¸öÖµÔ½´óÇø·Ö¶ÈÔ½´ó
		double factor_b = 1.5;  //Õâ¸öÖµÔ½´óÕûÌåÖµÔ½Ð¡
...
#pragma omp parallel for
  ...
  {
	  if(seg_method==0)
		  SegmentCloud(source_point_cloud, NewSourceCloud[i], NewSourceCloudIndex[i], source_sampleIndex[i], src_seg_nums, K, sample_num);  //°´ÕÕÊýÁ¿·Ö¿é
	  else 
		  SegmentCloud1(source_point_cloud, NewSourceCloud[i], NewSourceCloudIndex[i], source_sampleIndex[i], src_seg_nums, seg_radis, sample_num); //°´ÕÕ°ë¾¶·Ö¿é
  ...
  {
	  if (seg_method == 0)
		  SegmentCloud(target_point_cloud, NewTargetCloud[i], NewTargetCloudIndex[i], target_sampleIndex[i], tgt_seg_nums, K, sample_num);  //°´ÕÕÊýÁ¿·Ö¿é
	  else
		  SegmentCloud1(target_point_cloud, NewTargetCloud[i], NewTargetCloudIndex[i], target_sampleIndex[i], tgt_seg_nums, seg_radis, sample_num); //°´ÕÕ°ë¾¶·Ö¿é
  }
